...
RadialDistFunc::RadialDistFunc(boost::shared_ptr<Trajectory> traj, const std::string& file_name, double delR, double maxR, int nSkip)
   : Analyzer(traj), m_file_name(file_name), m_delR(delR), m_maxR(maxR), m_nSkip(nSkip)
   {
   }


void RadialDistFunc::evaluate() {  // need to add option to do g2(r) of specific types e.g. AA AB BB
    // skip nSkip frames for calculation
    // read the frames and make sure there is a simulation box
   unsigned int nSkip = m_nSkip;
   ...
   if (!m_traj->hasBox()) {
    ...
    unsigned int maxFrame = (frames.size()/nSkipP1)*nSkipP1 - 1; 
...
void export_RadialDistFunc() {
   ...
   class_<RadialDistFunc, boost::shared_ptr<RadialDistFunc>, bases<Analyzer>, boost::noncopyable >
   ("RadialDistFunc", init< boost::shared_ptr<Trajectory>, const std::string&, double, double, int >()); 
}
