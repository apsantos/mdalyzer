...
This toolbox will include methods to calculate the mean-square displacement, velocity autocorrelation function, density and temperature profiles, pair distribution function, and static structure factor.  We will also be able to identify the presence of clusters. This toolbox can be easily extended by the user to include other compute methods through the C++ API.
...
\item{\textbf{Velocity autocorrelation function.} Representative implementation of a \verb=Compute=. Needed to validate \verb=Trajectory.analyze()= loop is functional. Headed by Joey Vella.}
\end{itemize}

\subsection*{Alpha version (12 Dec 2014)}
\begin{itemize}
\item{\textbf{Additional readers completed.} XYZ file format (Joey Vella) and PDB file format (Sang Beom Kim). Allows for testing of trajectories compiled from different frames}
\item{\textbf{Additional computes completed.} Mean-square displacement (Andrew Santos) and pair distribution function (Elia Altabet).}
