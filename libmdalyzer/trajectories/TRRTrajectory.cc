 ...
 * particle positions.
 */
int TRRTrajectory::readFromFile(XDRFILE* trjFileXDR_, int& natoms_)
    {
    ...
    rvec* vel_i;                    // velocity
    std::vector< Vector3<double> > velocities(natoms_,
                                        Vector3<double>(0.,0.,0.));
    rvec* for_i;                    // force
...
void export_TRRTrajectory()
    ...
    class_<TRRTrajectory, boost::shared_ptr<TRRTrajectory>, bases<Trajectory>, boost::noncopyable >("TRRTrajectory");
    }
